    ...
    setTimeout(async () => {
      try {
        ...
        const homeAudit=await contents.executeJavaScript(`(async()=>{
          ...
          const waitFor=async(predicate,timeout=3000)=>{const deadline=performance.now()+timeout;while(!predicate()&&performance.now()<deadline)await new Promise(resolve=>setTimeout(resolve,20));return predicate();};
          ...
          document.getElementById('home-media-library').click();await pause();const sourceSelect=document.getElementById('music-source-select');let sourceReadyDeadline=performance.now()+3000;while(sourceSelect.disabled&&performance.now()<sourceReadyDeadline)await new Promise(resolve=>setTimeout(resolve,20));sourceSelect.value='catalog-test';sourceSelect.dispatchEvent(new Event('change',{bubbles:true}));let sourceSwitchDeadline=performance.now()+3000;while((audio.paused||document.getElementById('home-media-title').textContent!=='晨光'||document.getElementById('music-playlist-select').value!=='morning')&&performance.now()<sourceSwitchDeadline)await new Promise(resolve=>setTimeout(resolve,20));
          const firstPlaylistPlaying=!audio.paused&&document.getElementById('home-media-title').textContent==='晨光'&&document.getElementById('music-playlist-select').value==='morning';
          ...
          const musicPlaylistSwitchDebug={firstPlaylistPlaying,paused:audio.paused,title:document.getElementById('home-media-title').textContent,playlistId:playlistSelect.value,sourceLabel:document.getElementById('home-media-source-label').textContent,catalogHidden:document.querySelector('[data-music-source-panel="catalog"]').hidden,status:document.getElementById('home-media-status').textContent};
          ...
          input.value='流式测试';document.getElementById('home-chat-form').requestSubmit();await new Promise(resolve=>setTimeout(resolve,30));document.getElementById('home-chat-stop').click();await waitFor(()=>{const reply=[...document.querySelectorAll('#home-chat-messages [data-role="assistant"]')].at(-1);return reply?.dataset.state==='stopped'&&!input.readOnly&&document.getElementById('home-chat-form').dataset.busy==='false';});
          ...
          document.getElementById('home-chat-session-confirm-save').click();await waitFor(()=>document.getElementById('home-chat-session-state').textContent.includes('已保存到当前工作区')&&(JSON.parse(localStorage.getItem('notch-ai-chat-sessions-v1')||'{}').sessions||[]).length===1);const storedSessions=JSON.parse(localStorage.getItem('notch-ai-chat-sessions-v1')||'{}').sessions||[];const sessionSaved=storedSessions.length===1&&storedSessions[0].records.some(record=>record.sources?.[0]?.text==='冻结正文')&&storedSessions[0].records.some(record=>record.state==='stopped')&&storedSessions[0].records.some(record=>record.state==='error')&&document.getElementById('home-chat-session-state').textContent.includes('已保存到当前工作区');
